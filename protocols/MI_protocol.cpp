...
void MI_protocol::dump (void)
{
    ...
    fprintf (stdout, "MI_protocol - state: %s\n", block_states[state]);
#endif
}
...
inline void MI_protocol::do_snoop_M (Mreq *request)
{
    ...
    case GETM:
    	 ...
    	 * MOSI/MOESIF)
    	 */
    	/* NOTE: send_DATA_on_bus() implicitly calls set_shared_line() for you to let
		 * other caches know that the block is coming from a cache and not from memory i.e
		 * it is a $-$ transfer.*/

...
protocols=( MSI MESI MOSI MOESIF )
...
generate_stats() {
    ...
    local trace_directory="$(trace_path "$trace")"
    local output_file="$(student_stat_path "$protocol" "$trace")"
    
    if ! bash run.sh -p "$protocol" -t "$trace_directory" &> "$output_file"; then
